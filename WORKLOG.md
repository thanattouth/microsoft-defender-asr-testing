...
| 03 - Block execution of potentially obfuscated scripts | Ready | Retest pending | JScript v1 completed with marker created and no ASR event. Primary trigger upgraded to a large XOR-hex `eval()` blob with marker-only child-process behavior after comparison with Microsoft's official sample. |
