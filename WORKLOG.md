...
| 04 - Block persistence through WMI event subscription | Ready | Windows validation pending | Primary uses a never-triggered `CommandLineEventConsumer` subscription through PowerShell WMI APIs; fallback uses an equivalent MOF. Both use exact-name cleanup and require zero remaining WMI artifacts. |
